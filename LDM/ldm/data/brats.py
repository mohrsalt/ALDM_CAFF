...
brats_transforms = transforms.Compose(
    [
        ...
        transforms.CropForegroundd(keys=["t1n", "t1c", "t2w", "t2f"], source_key="t1", allow_missing_keys=True),
...
def get_brats_dataset(data_paths):
    ...
    for data_path in data_paths:
        for subject in os.listdir(data_path):
            ...
            t1n = os.path.join(sub_path, f"{subject}_t1n.nii.gz") 
            t1c = os.path.join(sub_path, f"{subject}_t1c.nii.gz") 
            t2w = os.path.join(sub_path, f"{subject}_t2w.nii.gz") 
            t2f = os.path.join(sub_path, f"{subject}_t2f.nii.gz") 
